...
import { fetchStudentById, addStudentOverride, deleteStudentOverride } from '../api/api';
...
const OverridesPage = () => {
  ...
  const [subForCourses, setSubForCourses] = useState(''); // Changed to plural
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      setError(`Failed to find student with ID ${studentId}.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddOverride = async (e) => {
    ...
    try {
      const response = await addStudentOverride(studentId, newOverride);
  ...
  const handleDeleteOverride = async (index) => {
    if (window.confirm('Are you sure you want to delete this override?')) {
      try {
        const response = await deleteStudentOverride(studentId, index);
